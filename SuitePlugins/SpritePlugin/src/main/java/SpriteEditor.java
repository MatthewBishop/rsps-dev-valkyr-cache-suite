...
public class SpriteEditor extends FXController {
	...
	private void addContextMenus() {
			...
			try {
				...
				if (id != -1) {
					...
					Task<Boolean> task = TaskUtil.create(() -> {
						byte[] data = sprite.encode(new OutputBuffer(16)).flip();
		...
		add.setOnAction((action) -> {
			try {
				...
				if (Objects.nonNull(file)) {
					...
					CacheLibrary.get().getIndex(getInfo().getIndex()).addArchive(container.id).addFile(0, container.encode(new OutputBuffer(16)).flip());
				}
			} catch (IOException e) {
		...
		replace.setOnAction((action) -> {
			try {
				...
				if (file != null) {
					...
					frames.getItems().setAll(container.toSpriteFrames());
					CacheLibrary.get().getIndex(8).addArchive(selected.id).addFile(0, container.encode(new OutputBuffer(16)).flip());
				}
				select_archive();
			} catch (IOException e) {
				e.printStackTrace();
		...
		delete.setOnAction((action) -> {
			...
			CacheLibrary.get().getIndex(8).addArchive(container.id).addFile(0, container.encode(new OutputBuffer(16)).flip());
			frames.getItems().setAll(container.toSpriteFrames());
		});
