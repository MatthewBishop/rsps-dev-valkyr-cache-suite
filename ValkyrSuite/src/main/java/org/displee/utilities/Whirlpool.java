...
public class Whirlpool {
	...
	static {
		for (int x = 0; x < 256; x++) {
			...
			long v9 = v8 ^ v1;
			aLongArrayArray8[0][x] = (v1 << 56 | v1 << 48 | v4 << 40 | v1 << 32 | v8 << 24 | v5 << 16 | v2 << 8 | v9);
			for (int t = 1; t < 8; t++) {
				aLongArrayArray8[t][x] = (aLongArrayArray8[t - 1][x] >>> 8 | aLongArrayArray8[t - 1][x] << 56);
			}
		}
		aLongArray9[0] = 0L;
		for (int r = 1; r <= 10; r++) {
			...
			aLongArray9[r] = (aLongArrayArray8[0][i] & ~0xffffffffffffffL
					^ aLongArrayArray8[1][i + 1] & 0xff000000000000L ^ aLongArrayArray8[2][i + 2] & 0xff0000000000L
					^ aLongArrayArray8[3][i + 3] & 0xff00000000L ^ aLongArrayArray8[4][i + 4] & 0xff000000L
					^ aLongArrayArray8[5][i + 5] & 0xff0000L ^ aLongArrayArray8[6][i + 6] & 0xff00L
	...
	protected void processBuffer() {
		...
		for (int r = 1; r <= 10; r++) {
			for (int i_0_ = 0; i_0_ < 8; i_0_++) {
				...
				int s = 56;
				while (t < 8) {
					aLongArray11[i_0_] ^= (aLongArrayArray8[t][(int) (aLongArray10[i_0_ - t & 0x7] >>> s) & 0xff]);
					t++;
					s -= 8;
				}
			}
			...
			for (int i_2_ = 0; i_2_ < 8; i_2_++) {
				...
				int s = 56;
				while (t < 8) {
					aLongArray11[i_2_] ^= (aLongArrayArray8[t][(int) (state[i_2_ - t & 0x7] >>> s) & 0xff]);
					t++;
					s -= 8;
				}
			}
	...
	public void NESSIEadd(byte[] source, long sourceBits) {
		...
		while (sourceBits > 8L) {
			int b = (source[sourcePos] << sourceGap & 0xff | (source[sourcePos + 1] & 0xff) >>> 8 - sourceGap);
			if (b < 0 || b >= 256) {
