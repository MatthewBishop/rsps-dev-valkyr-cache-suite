...
public class Index extends ReferenceTable {
	 ...
	 * 
	 * @param map A map of xteas (example: map.put(23, new int[4]) will create xteas
	 *            for archive 23).
	 * @return If this index was updated with success.
	 */
	...
	public boolean update(ProgressListener listener, Map<Integer, int[]> map) {
		...
		if (listener != null) {
			log.info("Updating checksum table...");
		...
		if (listener != null) {
			log.info("Successfully updated index {}", id);
			listener.notify(1, "Successfully updated index " + id + ".");
		}
		return true;
	}
			...
			try {
				...
				if (archiveSector.getSize() < 0) {
					return null;
						...
						.getPosition() > (super.origin.getMainFile().length() / Constants.ARCHIVE_SIZE)) {
					return null;
				}
				int read = 0;
				int chunk = 0;
				...
				while (read < archiveSector.getSize()) {
					...
					if (!archiveSector.read(inputBuffer)) {
						...
						throw new RuntimeException("Error, the read data is incorrect. Data[currentIndex=" + super.id
								+ ", index=" + archiveSector.getIndex() + ", currentId=" + id + ", id="
								+ archiveSector.getId() + ", currentChunk=" + chunk + ", chunk="
								+ archiveSector.getChunk() + "]");
					} else if (archiveSector.getNextPosition() < 0 || archiveSector
							.getNextPosition() > (super.origin.getMainFile().length() / Constants.ARCHIVE_SIZE)) {
		...
		synchronized (super.origin.getMainFile()) {
			try {
				...
				if (overWrite) {
					...
					position = (int) ((super.origin.getMainFile().length() + (Constants.ARCHIVE_SIZE - 1))
							/ Constants.ARCHIVE_SIZE);
					if (position == 0) {
						position = 1;
					}
				}
				...
				while (written < data.length) {
					...
					if (overWrite) {
						...
						if (currentPosition < 0
								|| super.origin.getMainFile().length() / Constants.ARCHIVE_SIZE < currentPosition) {
							return false;
						}
					}
					if (currentPosition == 0) {
						overWrite = false;
						currentPosition = (int) ((super.origin.getMainFile().length() + (Constants.ARCHIVE_SIZE - 1))
								/ Constants.ARCHIVE_SIZE);
						if (currentPosition == 0) {
							currentPosition++;
						}
