...
public class ItemDefinition implements AbstractDefinition, Cloneable {
	...
	public void decode(InputStream buffer) {
		while (true) {
			...
			if (opcode == 1) {
				this.inventory_model = buffer.readBigSmart();
			} else if (opcode == 2) {
				...
				this.maleModel0 = buffer.readBigSmart();
			} else if (opcode == 24) {
				this.maleModel1 = buffer.readBigSmart();
			} else if (opcode == 25) {
				this.femaleModel0 = buffer.readBigSmart();
			} else if (opcode == 26) {
				this.femaleModel1 = buffer.readBigSmart();
			} else if (opcode == 27) {
				...
				if (opcode == 40) {
					...
					this.maleModel2 = buffer.readBigSmart();
				} else if (opcode == 79) {
					this.femaleModel2 = buffer.readBigSmart();
				} else if (opcode == 90) {
					this.maleHeadModel0 = buffer.readBigSmart();
				} else if (opcode == 91) {
					this.femaleHeadModel0 = buffer.readBigSmart();
				} else if (opcode == 92) {
					this.maleHeadModel2 = buffer.readBigSmart();
				} else if (opcode == 93) {
					this.femaleHeadModel2 = buffer.readBigSmart();
				} else if (opcode == 95) {
	...
	public byte[] encode() {
		...
		if (this.maleModel0 != -1) {
			stream.writeByte(23);
			stream.writeBigSmart(this.maleModel0);
		}

		if (this.maleModel1 != -1) {
			stream.writeByte(24);
			stream.writeBigSmart(this.maleModel1);
		}

		if (this.femaleModel0 != -1) {
			stream.writeByte(25);
			stream.writeBigSmart(this.femaleModel0);
		}

		if (this.femaleModel1 != -1) {
			stream.writeByte(26);
			stream.writeBigSmart(this.femaleModel1);
		}
		...
		for (index = 0; index < 4; ++index) {
			if (this.inventory_options[index] != null
					&& (index != 4 || !this.inventory_options[index].equals("drop"))) {
		...
		if (this.maleModel2 != -1) {
			stream.writeByte(78);
			stream.writeBigSmart(this.maleModel2);
		}

		if (this.femaleModel2 != -1) {
			stream.writeByte(79);
			stream.writeBigSmart(this.femaleModel2);
		}
	...
	public boolean save(CacheLibrary cache) {
		cache.getIndex(19).addArchive(Utils.getConfigArchive(id, 8)).addFile(Utils.getConfigFile(id, 8), encode());
	...
	public String toString() {
		return this.id + " - " + this.name
				+ (Constants.settings.debug ? "(" + (this.noteTemplate > 0 ? "Noted" : "Unnoted") + ")" : "");
	}
	...
	public void applyEquipData() {
		...
		if (name.contains("chestplate") || name.contains("body") || name.contains("brassard") || name.contains("top")
				|| name.contains("jacket") || name.contains("shirt") || name.contains("apron") || name.contains("coat")
				|| name.contains("blouse") || name.contains("hauberk") || name.contains("chestguard")
				|| name.contains("torso") || name.contains("garb") || name.contains("tunic")
				|| name.contains("armour")) {
			equipSlot = 4;
			equipType = 6;
		}

		if (name.contains("legs") || name.contains("skirt") || name.contains("robe bottom") || name.contains("chaps")
				|| name.contains("chainskirt") || name.contains("leggings") || name.contains("tassets")
				|| name.contains("slacks") || name.contains("bottoms") || name.contains("trousers")
				|| name.contains("greaves") || name.contains("Shorts")) {
			equipSlot = 7;
		}

		if (name.contains("mask") || name.contains("helm") || name.contains("hat") || name.contains("hood")
				|| name.contains("coif") || name.contains("mitre") || name.contains("eyepatch") || name.contains("mask")
				|| name.contains("boater") || name.contains("beret") || name.contains("snelm") || name.contains("tiara")
				|| name.contains(" ears") || name.contains("head") || name.contains("cavalier")
				|| name.contains("wreath") || name.contains("fedora") || name.contains("fez")
				|| name.contains("headband") || name.contains("headgear") || name.contains("faceguard")) {
		...
		if (name.contains("amulet") || name.contains("necklace") || name.contains("pendant") || name.contains("scarf")
				|| name.contains("stole") || name.contains("symbol")) {
			equipSlot = 2;
		}

		if (name.contains("boots") || name.contains("feet") || name.contains("shoes") || name.contains("sandals")) {
			equipSlot = 10;
		}

		if (name.contains("gloves") || name.contains("vamb") || name.contains("bracelet") || name.contains("bracers")
				|| name.contains("gauntlets") || name.contains("cuffs") || name.contains("hands")
				|| name.contains("armband")) {
			equipSlot = 9;
		}
		...
		if (name.contains("shield") || name.contains("defender") || name.contains("book") || name.contains(" ward")
				|| name.equalsIgnoreCase("tome of fire") || name.equalsIgnoreCase("toktz-ket-xil")
				|| name.contains("satchel")) {
			equipSlot = 5;
		}

		if (name.contains("arrow") || name.contains("bolts") || name.contains("brutal") || name.contains("arrows")
				|| name.contains("tar") || name.contains(" blessing") || name.contains("javelin")
				|| name.contains("grapple")) {
			equipSlot = 13;
		}

		if (name.contains("sword") || name.contains("whip") || name.contains("halbard") || name.contains("claws")
				|| name.contains("spear") || name.contains("anchor") || name.contains("dagger") || name.contains("bow")
				|| name.contains("bulwark") || name.contains("axe") || name.contains("maul")
				|| name.contains("ballista") || name.contains("club") || name.contains("katana")
				|| name.contains("scythe") || name.contains("staff") || name.contains("wand") || name.contains("lizard")
				|| name.contains("salamander") || name.contains("flail") || name.contains("mjolnir")
				|| name.contains("mace") || name.contains("hammer") || name.contains("arclight")
				|| name.contains("crozier") || name.contains("banner") || name.contains("dart") || name.contains("cane")
				|| name.contains("chinchompa") || name.contains("knife") || name.contains("scimitar")
				|| name.contains("hasta") || name.contains("rapier") || name.contains("sickle")
				|| name.contains("greegree") || name.contains("machete") || name.contains("blackjack")
				|| name.contains("blowpipe") || name.contains("trident") || name.contains("2h")
				|| name.contains("halbard") || name.contains("spear") || name.contains("bulwark")
				|| name.contains("maul") || name.contains(" ballista") || name.contains("salamander")
				|| name.contains("lizard")
				|| (name.contains("bow") && (!name.contains("cross") && !name.contains("karil's")))
				|| name.contains("club") || name.contains("claws") || name.contains("blowpipe")
				|| name.contains("anchor") || name.contains("greataxe") || name.contains("godsword")) {
			equipSlot = 3;
			if (name.contains("2h") || name.contains("halbard") || name.contains("spear") || name.contains("bulwark")
					|| name.contains("maul") || name.contains(" ballista") || name.contains("salamander")
					|| name.contains("lizard")
					|| (name.contains("bow") && (!name.contains("cross") && !name.contains("karil's")))
					|| name.contains("club") || name.contains("claws") || name.contains("blowpipe")
