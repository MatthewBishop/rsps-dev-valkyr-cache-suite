...
public class FontMetrics implements AbstractDefinition, Cloneable {
			...
			for (int index = 0; index < 256; index++) {
				if (index != 32) {
					for (int i_56_ = 0; i_56_ < 256; i_56_++) {
						if (i_56_ != 32)
							aByteArrayArray6198[index][i_56_] = (byte) method392(is_47_, is_51_, is_44_, aByteArray6199,
									is_43_, index, i_56_);
					}
				}
			}
			verticalSpacing = (is_44_[32] + is_43_[32]);
		} else {
	...
	private static int method392(byte[][] is, byte[][] is_1_, int[] is_2_, byte[] is_3_, int[] is_4_, int index,
			int i_5_) {
