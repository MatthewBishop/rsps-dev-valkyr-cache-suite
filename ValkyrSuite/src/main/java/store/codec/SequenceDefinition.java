...
public class SequenceDefinition implements AbstractDefinition, Cloneable {
			...
			if (1 == opcode) {
				...
				for (int i_12_ = 0; i_12_ < i_11_; i_12_++) {
					...
					if (i_13_ > 0) {
						...
						sounds[i_12_][0] = stream.read24BitInt();
				...
				if (params == null) {
					params = new HashMap<Object, Long>(size);
				}
	...
	public boolean save(CacheLibrary cache) {
		cache.index(20).addArchive(Utils.getConfigArchive(id, 7)).add(Utils.getConfigFile(id, 7), encode());
		return cache.index(20).update();
	}
