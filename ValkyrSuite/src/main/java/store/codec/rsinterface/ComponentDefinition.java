...
public class ComponentDefinition {
	...
	private static IComponentMasks GLOBAL_SETTINGS = new IComponentMasks(0, -1);
	/**
	 * positions
	 */
	...
	public IComponentMasks activeProperties;
	public int type;
	...
	public static ComponentDefinition[] getInterface(int id, boolean reload) {
		...
		if (icomponentsdefs[id] == null || reload) {
			...
			for (int i = 0; i < icomponentsdefs[id].length; i++) {
				byte[] data = ValkyrCacheLibrary.get().index(3).archive(id).file(i).getData();
				if (data != null) {
	...
	final void decode(InputBuffer stream, int id, int interfaceId) {
		...
		// System.out.println(this.interfaceId+" "+this.componentId+" = menuCursorMask:
		// "+menuCursorMask+ " menuOptionsCount: "+menuOptionsCount);
		if (options_length > 0) {
		...
		activeProperties = new IComponentMasks(optionMask, mask);
		if (newInt >= 0) {
	...
	public static ArrayList<ComponentDefinition> getInterfaceContainers(int interfaceId) {
		...
		for (ComponentDefinition component : possibleParents) {
			...
			if (component.type == ComponentConstants.CONTAINER
					&& ComponentDefinition.hasChilds(interfaceId, component.ihash)) {
	...
	public static int getInterfaceDefinitionsSize() {
		return ValkyrCacheLibrary.get().index(3).last().getId() + 1;
	}

	public static int getInterfaceDefinitionsComponentsSize(int interfaceId) {
		return ValkyrCacheLibrary.get().index(3).archive(interfaceId).last().getId() + 1;
	}
