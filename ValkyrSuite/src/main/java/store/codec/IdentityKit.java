...
public class IdentityKit implements AbstractDefinition {
			...
			if (opcode == 1) {
				...
				if (2 == opcode) {
					...
					for (index = 0; index < length; ++index) {
						this.bodyModels[index] = stream.readBigSmart();
					}
				} else if (opcode == 3) {
					...
					if (opcode >= 60 && opcode < 70) {
						this.headModels[opcode - 60] = stream.readBigSmart();
					}
				}
			}
		}
	...
	public byte[] encode() {
		...
		if (bodyModels != null) {
			...
			for (int index = 0; index < bodyModels.length; index++) {
				stream.writeBigSmart(bodyModels[index]);
			}
		}
	...
	public boolean save(CacheLibrary cache) {
		cache.getIndex(2).addArchive(3).addFile(id, encode());
