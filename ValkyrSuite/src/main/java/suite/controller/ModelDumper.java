		...
		try {
			...
			if (modelIds == null) {
				...
				for (int index = 0; index < models.length; index++) {
					if (Selection.cache.getIndex(7).getArchive(models[index]).file(0) == null)
						continue;
					byte[] data = Selection.cache.getIndex(7).getArchive(Integer.parseInt(models[index])).file(0)
							.getData();
				...
				for (int index = 0; index < modelIds.length; index++) {
					if (Selection.cache.getIndex(7).getArchive(modelIds[index]).file(0) == null)
						continue;
					byte[] data = Selection.cache.getIndex(7).getArchive(modelIds[index]).file(0).getData();
					DataOutputStream dos = new DataOutputStream(
