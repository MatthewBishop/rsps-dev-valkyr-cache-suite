...
public class SpotAnimations {
	...
	@FXML
	private ListView<SpotDefinition> graphics_list;
	@FXML
	private TextField modelId, animation, resizeX, resizeY, rotation, ambient, contrast, recoloring, retexturing,
			aByte5216, anInt5203, search_bar, aBoolean5215;
		...
		create.setOnAction((ActionEvent event) -> {
			...
			try {
				String input = JOptionPane.showInputDialog(null,
						"Please input the desired id, or leave blank for default id.");
				if (input.isEmpty())
					blank_def.id = SpotLoader.getDefinitions().length;
				else {
					int id = Integer.parseInt(input);
					if (cache.index(21).getArchive(Utils.getConfigArchive(id, 8))
