...
public class ParticleEditor {
	...
	private CacheLibrary cache = Selection.cache;
	private CustomTab tab;
	private ObservableList<ParticleDefinition> definitions;
	private ParticleDefinition def;

	@FXML
	private ListView<ParticleDefinition> particles;
	@FXML
	private TextField minimum_angle_h, maximum_angle_h, minimum_angle_v, maximum_angle_v, minimum_speed, maximum_speed,
			minimum_lifespan, maximum_lifespan, minimum_spawn_rate, maximum_spawn_rate, minimum_size, maximum_size,
			lifetime, size_change, change_speed, end_speed, fading_color, fading_alpha, anInt1873, anInt1855, anInt1819,
			anInt1847, anInt1850, anInt1851, anInt1852, anInt1840, search_bar;
	@FXML
	private CheckBox active_first, periodic, uniform_color_variance, aBoolean1877, aBoolean1863, aBoolean1818,
			aBoolean1862, aBoolean1874;
	@FXML
	private ColorPicker minimum_start_color, maximum_start_color;
	...
	public void initialize(CustomTab tab, boolean refresh) {
		...
		create.setOnAction((ActionEvent event) -> {
			...
			try {
				...
				InformationDialogue.create("Error", "An error has occured.", AlertType.ERROR,
	...
	public void save() {
		try {
			...
			java.awt.Color min_color = new java.awt.Color((float) minimum_start_color.getValue().getRed(),
					(float) minimum_start_color.getValue().getGreen(), (float) minimum_start_color.getValue().getBlue(),
			...
			java.awt.Color max_color = new java.awt.Color((float) maximum_start_color.getValue().getRed(),
					(float) maximum_start_color.getValue().getGreen(), (float) maximum_start_color.getValue().getBlue(),
